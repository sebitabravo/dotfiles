...
SCRIPT="$ROOT/config/claude/scripts/check-provider-runtime-parity.sh"
...
printf '%s' "$initial_json" | jq -e '.parity == false and .failures == 7' >/dev/null
...
for overlay in deepseek.settings.json glm.settings.json kimi.settings.json minimax.settings.json ollama.settings.json openrouter.settings.json qwen.settings.json; do
...
for overlay in kimi.settings.json ollama.settings.json qwen.settings.json; do
...
before=$(shasum -a 256 "$RUNTIME/qwen.settings.json" | awk '{print $1}')
tmp_json="$RUNTIME/qwen.settings.json.tmp"
jq '. + {runtimeOnly: true}' "$RUNTIME/qwen.settings.json" >"$tmp_json"
mv "$tmp_json" "$RUNTIME/qwen.settings.json"
drift_json=$(CLAUDE_RUNTIME_DIR="$RUNTIME" "$SCRIPT" --json)
printf '%s' "$drift_json" | jq -e '[.results[] | select(.path == "qwen.settings.json" and .status == "DRIFT")] | length == 1' >/dev/null
after=$(shasum -a 256 "$RUNTIME/qwen.settings.json" | awk '{print $1}')
[ "$before" != "$after" ]

CLAUDE_RUNTIME_DIR="$RUNTIME" "$SCRIPT" >/dev/null 2>&1 || true
still=$(shasum -a 256 "$RUNTIME/qwen.settings.json" | awk '{print $1}')
[ "$after" = "$still" ]
