...
fix(tenant): correct domain validation
...
# 4. Create PR toward development
...
```bash
# Verify tests pass (find the command in the project docs)
...
# 3. Fix, test, commit
git commit -m "fix(scope): description of the resolved bug"
...
❌ NEVER: git reset --hard (without checking state)
