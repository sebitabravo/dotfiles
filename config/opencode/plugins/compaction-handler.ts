...
export const CompactionHandler: Plugin = async ({ $ }) => {
  return {
    ...
    "session.compacted": async (_input, _output) => {
      ...
      try {
        await $`command -v engram && engram mem save --title "Compaction checkpoint" --content "Session compacted at $(date -u +%Y-%m-%dT%H:%M:%SZ)" --type discovery --project opencode 2>/dev/null || true`
      } catch {
