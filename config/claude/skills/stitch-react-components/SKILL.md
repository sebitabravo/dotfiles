...
description: >
  ...
  Use when converting a Stitch design into Vite/React components or synchronizing an existing implementation with Stitch.
allowed-tools:
  - "ToolSearch"
  - "Bash"
  - "Read"
...
The reliable downloader is bundled with this installed skill. Do not assume
that the current project directory contains it:

```bash
SKILL_DIR="${STITCH_REACT_COMPONENTS_SKILL_DIR:-$HOME/.claude/skills/stitch-react-components}"
FETCH_STITCH="$SKILL_DIR/scripts/fetch-stitch.sh"
```

## Phase 1: Retrieval and networking

> **GATE: Phase 1 is complete ONLY when all screens have been downloaded via `$FETCH_STITCH` AND visually audited. Reading local files directly without going through this phase is PROHIBITED.**

1. **Namespace discovery**: Run `ToolSearch` to find the Stitch MCP prefix. Use this prefix (e.g., `stitch:`) for all subsequent calls.
...
4. **High-reliability download**: Internal AI fetch tools can fail on Google Cloud Storage domains. You MUST use the provided script.
   - **HTML**: `bash "$FETCH_STITCH" "[htmlCode.downloadUrl]" ".stitch/designs/{page}.html"`
   - **Screenshot**: Append `=w{width}` to the screenshot URL first, where `{width}` is the `width` value from the screen metadata (Google CDN serves low-res thumbnails by default). Then run: `bash "$FETCH_STITCH" "[screenshot.downloadUrl]=w{width}" ".stitch/designs/{page}.png"`
...
1. **Environment setup**: If `node_modules` is missing, ask for permission first, then run `npm ci --ignore-scripts` inside `$SKILL_DIR` to enable the locked validation tools. Do not install these dependencies into the target project.
