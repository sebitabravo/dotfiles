...
For a full-backlog audit, first define the read-only inventory and disposition scope; this skill governs how resulting fixes are designed, batched, and closed.
...
- Re-verify every "fixed" or "still broken" claim against current `origin/main` THE DAY you act, and compare the report date against the fix merge date. A reproduction filed before the fix landed is evidence about an old build, not about current code — treating it as current is how a cluster gets re-opened around a defect that no longer exists. The inverse also holds: a fix merged after the last triage pass can close issues nobody re-checked.
- Close on exactly one named rule, and say which: (A) fixed on main — cite the commit AND the test that proves it; (B) superseded by a recorded maintainer decision; (C) the surface no longer exists; (D) duplicate of an already-fixed issue. If none of the four applies cleanly, it goes on a borderline list instead. When in doubt, do not close: a wrongly closed issue costs the reporter a second filing and you the trust to triage the next one.
