...
### 4b) Server-Timing headers (Performance Lab)

When the Performance Lab plugin is available, backend timings are exposed as `Server-Timing`
response headers — no browser and no UI needed, just inspect the headers.

Read:
- `references/server-timing.md`
