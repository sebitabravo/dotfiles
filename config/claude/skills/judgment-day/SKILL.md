...
- Resolve matching project skills before starting and pass the same paths to both independent `code-reviewer` invocations and the bounded `debugger` fix actor.
- Build one complete immutable target, then launch two independent `code-reviewer` judges in parallel with identical scope and criteria. The caller must explicitly require read-only behavior and the JSON findings contract for both invocations.
...
- Only the parent orchestrator merges/persists findings, launches the bounded `debugger` fix actor, and launches scoped re-judgment. The fix invocation must be labelled `JUDGMENT_DAY_FIX`. `security-auditor` is an optional third lens for auth, secrets, and permissions; it does not replace either judge.
...
2. Launch two independent read-only `code-reviewer` invocations in parallel, labelled Judge A and Judge B, against the same immutable target.
3. Merge findings into the frozen ledger and persist it through the selected artifact store.
4. Ask before round-one correction; run `debugger` in `JUDGMENT_DAY_FIX` mode only for confirmed severe IDs.
5. Run both `code-reviewer` judges again only over the frozen ledger plus immutable fix delta.
...
- The ordinary RDD receipt gate remains separate and is governed by this configuration and `rdd-defect-workflow`; Judgment Day never grants delivery authority.
