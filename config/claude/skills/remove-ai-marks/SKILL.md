...
This skill follows the upstream **thin-client** contract: the agent calls the
cleaning service over HTTP rather than invoking cleaning code as part of the
workflow. This dotfiles bundle vendors the current core service under
`scripts/`, so a local install remains self-contained; optional heavy backends
are still not bundled and are only used when the service advertises them.

## Local bundled service

The default dotfiles install path is `~/.claude/skills/remove-ai-marks/scripts`.
Override it with `REMOVE_AI_MARKS_SCRIPTS` when running from the repository or a
non-default install:

```bash
SCRIPTS="${REMOVE_AI_MARKS_SCRIPTS:-$HOME/.claude/skills/remove-ai-marks/scripts}"
```

Start the local core service (Python 3.10+ standard library only) in a separate
terminal when no external service is already running:

```bash
python3 "$SCRIPTS/server.py" --host 127.0.0.1 --port 8765
```

The service is intentionally not auto-started by the skill. Check `/health`
first and report the explicit start command if it is unavailable.
...
published GHCR image) or locally with the bundled command above. **Always check
it first**, and stop with a clear message if it is unreachable — never fall
back to an alternate cleaning path:
...
Or against a local checkout of the repo: `python3 "$SCRIPTS/audit_dir.py" DIR --json`.
...
(`docker compose up -d`, `python3 "$SCRIPTS/server.py" --host 127.0.0.1 --port 8765`, or the published GHCR image). Do **not**
