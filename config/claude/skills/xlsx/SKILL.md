...
> Check `openpyxl`, `pandas`, and `markitdown` before use; this skill does not assume optional packages are provisioned. If an import or command is missing, ask before provisioning and use `uv run --no-project --with ...` or the project environment — never install globally with `pip`.

> Script paths below are relative to this skill's directory. When the current
> working directory is the target project, set the installed skill root first:
>
> ```bash
> SKILL_DIR="${XLSX_SKILL_DIR:-$HOME/.claude/skills/xlsx}"
> ```
...
`openpyxl`, `pandas`, `markitdown` (optional Python dependencies; provision with `uv --with` or the project environment only after consent) · LibreOffice (`soffice`, auto-configured for sandboxed environments via `scripts/office/soffice.py`)
