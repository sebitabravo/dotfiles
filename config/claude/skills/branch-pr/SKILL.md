...
When the target repository uses a protected `main` and a `development` branch,
create feature work from `main`, target pull requests at `development`, and
never push directly to either shared branch:
...
Use the repository's ticket convention when one exists (for example,
`feature/TECH-XX-short-description`). Keep local WIP commits private or
temporary; do not rewrite shared history or use `git push --force`.
