...
5. In apply mode, edit only safe issues, preserve content, create supporting files when needed, then update the tracked registry or use the project-native registry command after checking `git status` and confirming that the command will not create an unsolicited commit.
