...
Paths are relative to this skill's directory. In the installed dotfiles runtime,
set the root explicitly when the current working directory is the target project:

```bash
SKILL_DIR="${PPTX_SKILL_DIR:-$HOME/.claude/skills/pptx}"
```

Use `"$SKILL_DIR/scripts/..."` when invoking a helper from another directory.
...
`pptxgenjs` is declared in this skill's local `package.json`, but `node_modules` is not assumed to exist. Check the skill-local dependency first. If it is missing, ask before provisioning and run `npm ci --ignore-scripts` from `$SKILL_DIR`; never install it globally or into the target project. The model knows the API; these are the footguns:
...
- **Icons:** render `react-icons` to SVG (`ReactDOMServer.renderToStaticMarkup`), rasterize with `sharp` at ≥256px, and insert via `addImage({ data: "image/png;base64," + buf.toString("base64") })`. The `image/png;base64,` prefix is required. `react-icons`, `react`, `react-dom`, and `sharp` are optional; ask before provisioning them in the skill-local environment and use `npm ci --ignore-scripts` only when the lock/package manifest covers the required workflow. Never install into the target project.
...
## Dotfiles dependency and parser safety

- `pptxgenjs@4.0.1` declares `image-size`, whose ICNS/JXL/HEIF parsers have
  known infinite-loop advisories (`GHSA-w3rx-r6r6-pgpr` and
  `GHSA-5p2g-fcmc-qvqq`). Do not import `image-size` directly or feed ICNS,
  JXL, HEIF, or HEIC assets into this workflow; prefer trusted PNG, JPEG, or
  SVG assets.
- Keep `npm audit --package-lock-only --ignore-scripts` visible until the
  package chain ships a patched dependency. A clean helper smoke does not
  erase the dependency advisory.
- The local guides `editing.md` and `pptxgenjs.md` remain available for the
  detailed template and from-scratch workflows; the current upstream rules
  above are the default contract.
...
`pptxgenjs` (npm, skill-local `package.json`; provision only after consent with `npm ci --ignore-scripts`) · `markitdown[pptx]`, `Pillow`, `defusedxml`, `lxml` (optional Python dependencies; provision with `uv --with` or the project environment only after consent) · LibreOffice (`soffice`, auto-configured for sandboxed environments via `scripts/office/soffice.py`) · `pdftoppm` (Poppler)
