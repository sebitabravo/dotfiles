---
name: handoff
description: Creates a HANDOFF.md file with the current project state for a clean handoff between sessions. Use when the session is long, the model is going in circles on the same solution, or before clearing the session. Codex loads it through the SessionStart hook.
---
